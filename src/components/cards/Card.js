...
export const Card = ({ product, discount, ...rest }) => (
  <div className="card">
    ...
    <div className="slicker__paragraph">
      <div className="product__category__card">{product[0].category}</div>
      <div className="product_desc">{product[0].name}</div>
      <div className="product_desc">{product[0].price}</div>
    </div>
  </div>
);
